...
These are some modules modules to illustrate vector operations.
...
#+begin_src scad :eval no
  draw_line(point1, point2, line_thickness, extend = false, multiplication_factor = 1);
#+end_src
...
#+begin_src scad :file ./images/fig_line_demo.png :colorscheme Cornfield :size 400,400 :projection ortho :exports both
  ...
  color("Green") translate([10, 0, 0]) {
    draw_line(p1, p2, extend = true, multiplication_factor = 0.5);
  }
#+end_src
...
#+begin_src scad :eval no
  draw_vector(point1, point2, line_thickness);
#+end_src
...
#+begin_src scad :eval no
  draw_parallelepiped(point1, point2, line_thickness);
#+end_src
...
#+begin_src scad :eval no
  draw_vector_components(point1, point2, thickness, bounding_box = false);
#+end_src
...
#+begin_src scad :eval no
  vector_addition(point1, point2);
#+end_src

Parameters:

- ~point1, point2~: Are either one-, two- or three-dimensional vectors.
...
#+begin_src scad :file ./images/fig_calculate_the_vector_addition.png :colorscheme Cornfield :size 400,400 :projection ortho :exports both
  ...
  // Drawing vector p1 from the origin
  color("Red", 1.0) draw_vector(origin, p1);
  // Drawing vector p2 from the tip of vector p1
  translate(p1) color("Green", 1.0) draw_vector(origin, p2);
  // Drawing the computed vector v from the origin. The tip of v lands on the tip of p2  
...
#+begin_src scad :file ./images/fig_vector_substraction_example.png :colorscheme Cornfield :size 400,400 :projection ortho :exports :exports both
  ...
  // Compute the negative of p2
  np2 = (-1)*p2;

  // Compute p1+(-p2) -> p1-p2 and store the value in variable v
  v = vector_addition(p1, np2);

  // Setting the reference for the two vectors
  origin = [0, 0, 0];

  // Draw vector p1 fom the origin
  color("Red", 1.0) draw_vector(origin, p1);
  // Draw vector p2 from the tip of p1
  translate(p1) {
    color("Green", 1.0) draw_vector(origin, np2);
  }
  // Dra the computed vector v from the origin. The tip of v lands on the tip of p2  
  color("Yellow", 1.0) draw_vector(origin, v);
#+end_src
